label aftergame216:   
    ...
    if rep >= 1:
        "Счастливо нурик уходит из 116." (what_slow_cps=25)
    else:
        "О нет. Кажется ему отшибло память и он стал первокурсником." (what_slow_cps=25)
...
image larionov1 = Transform("characters/lar.png", zoom = 0.1)
...
label chapter4_start:
    ...
    scene black_back
    show text "4 пара" at truecenter with fade
    pause 1.5
    hide text with fade
    ...
    "Боль резко пронзила голову Нурика. Он лежал на полу, глаза щурились от яркого света..." (what_slow_cps=25)
    
    show nurik at left_side1 with dissolve
    N "Какого (язык хештегов)?"(what_slow_cps=25)
    
    "После не самого нежного пробуждения раскалывается голова. Четвертая пара тоже совсем не подходит для работы" (what_slow_cps=25)
    
    "Нурик дергает ручку двери." (what_slow_cps=25)

    show larionov1 at right_big
    L "Эх, а могли бы сделать фласк на первом курсе... теперь придется отдуваться." (what_slow_cps=25)
    L "Садитесь поближе, сейчас все быстро расскажу и будете работать." (what_slow_cps=25)
    
    scene bg poblighe with dissolve
    show larionov1 at right_big
    L "При использовании HTTP-протокола простейший способ аутентификации — Basic access authentication. В принципе этот протокол устарел и-" (what_slow_cps=140)

    L "Все понятно?"
    hide nurik
    show nurik_udivil at left_side
    pause 1.0
    N "..."
    
    show larionov1 at right_big with dissolve
    L "Первым, что при обращении к защищенному ресурсу сервер выдаст пользователю, не имеющему доступа, будет ошибка 401 Unauthorized. При этом ответ также-" (what_slow_cps=120)

    
    "Дмитрий Ильич с особым мастерством проходится...нет, пробегается по теме. И, хотя в глазах одногруппников понимания меньше, чем страха, все кивают." (what_slow_cps=25)
    
    "Что ж, нужно выбираться из плачевного положения." (what_slow_cps=25)
    
    hide nurik_udivil
    show nurik_cool at left_side1 with dissolve
    
    N "Дмитрий Ильич, вы такой же быстрый, как D2..." (what_slow_cps=25)
    
    L "Что за бред? D1 быстрее!" (what_slow_cps=25)
    N "Нет, D2 намного круче чем D1! Вы видели как там мои ласточки летают? Готов поспорить, что они быстрее." (what_slow_cps=25)
    
    L "Ты сейчас договоришься, каракалпакский мальчик." (what_slow_cps=25)
    ...
    menu:
        "Настоять на своем мнении о D2":
            ...
            $ rep -= 1
            show larionov1 at right_big 
            L "Я же говорил!" (what_slow_cps=25)
            play sound "xbox.mp3"
            $ renpy.call_screen("scr_achievement_get", title="", a_text="Дестрой пак🍺", icon="GERB.png")
           
            "Нурик переоценил свои способности. Теперь, помимо основной работы, он получит наказание." (what_slow_cps=25)
        ...
        "Согласиться с Дмитрием Ильичом":
            ...
            $ rep += 1
            show larionov1 at right_big with dissolve
            L "Так уж и быть..." (what_slow_cps=25)
            "Ларионов теряет интерес, и Hурик может выдохнуть" (what_slow_cps=25)
            scene black_back with fade
            jump aftergame216
